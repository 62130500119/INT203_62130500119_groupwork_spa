<template>   <div class="w-screen h-screen pt-36 bg-blue-100">
    <div class="container middlex">
      <form @submit.prevent="submitForm">
        ...
        <div class="middlex">
          <p v-if="invalidNameInput" class="invalid">Please enter your Name!</p>
        </div>
        ...
        <input-box v-show="pet">
          <p class="text-white">
            Your pet is
            <span class="text-xl font-bold text-white md:uppercase">{{
              pet
            }}</span>
          </p>
        </input-box>
        <div class="middlex">
          <p v-if="invalidPetInput" class="invalid">Please choose your Pet!</p>
        </div>
        <div class="right">
      ...
      <div class="result">
        <ul v-for="survey in petSurvey" :key="survey.id">
          <ResultBox :sname="survey.name">
            ...
            <button @click="showData(survey)" class="bg-blue-500 m-1">
...
export default {
  ...
  methods: {
    ...
    getImgSrc(survey) {
      return survey.src == "../assets/dog.jpg"
    ...
    async deleteSurvey(deleteId) {
      try {
        ...
        this.petSurvey = this.petSurvey.filter(
      ...
      try {
        const res = await fetch(`${this.url}/${editingSurvey.id}`, {
          ...
          body: JSON.stringify({
            ...
            pet: editingSurvey.pet,
            src: editingSurvey.src,
          }),
        });
        const data = await res.json();
        this.petSurvey = this.petSurvey.map((survey) =>
          survey.id === editingSurvey.id
            ? { ...survey, name: data.name, pet: data.pet, src: data.src }
            : survey
        );
